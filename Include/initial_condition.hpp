...
void Initial_Condition(int polymer_length,int number_of_polymers){
		...
		mc_move.push_back(MC_move(1,1,0));
		mc_move.push_back(MC_move(-1,-1,0));
		mc_move.push_back(MC_move(1,-1,0));
		mc_move.push_back(MC_move(-1,1,0));
		...
		while(number_of_created_polymer < number_of_polymers){
			...
			//std::cout<<lattice[t][u][v]<<"\t"<<"Even ? "<<(t+u+v)%2<<"\t"<<t<<"\t"<<u<<"\t"<<v<<std::endl;
			...
			for(int j=1;j<int(polymer_length);j++){
				...
				while(occupation != 0 && n_trials < 24){
					auto MC_move = rand()%12; //there are 12 possible movement
				...
				if(n_trials <= 24 && occupation == 0){
					...
					lattice[monomer_tmp.t_image][monomer_tmp.u_image][monomer_tmp.v_image]+=1;
					n_monomers+=1;
					n_monomers_no_stored+=1;
					temp_stored+=1;
					//std::cout<<temp_stored<<std::endl;
					if(temp_stored==stored_length && n_monomers!=polymer_length){
					...
					//std::cout<<"Break at "<<number_of_created_polymer<<std::endl;
					break;
				}
			
			}
			if(n_monomers==polymer_length){
				std::cout<<number_of_created_polymer<<"\t"<<n_monomers_no_stored<<std::endl;
		...
		for(auto i=0;i<Lattice_size;i++){
			for(auto j=0;j<Lattice_size;j++){
				for(auto k=0;k<Lattice_size;k++){
					if(lattice[i][j][k] != 2 && lattice[i][j][k] != 0 && lattice[i][j][k] != 1){
						condition=false;
